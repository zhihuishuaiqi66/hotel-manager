...
<div class="modal">
    ...
    <div class="success" id="successMsg">
        ...
        <h3>预订成功！</h3>
            ...
            <button class="btn-outline" onclick="resetForm()">再填一单</button>
            <button class="btn-outline" style="margin-left:10px;background:var(--s);color:#fff" onclick="location.href='/breakfast'">返回首页</button>
    </div>
</div>
...
<div class="hd"><div class="hdc"><h1>后台管理</h1><div class="st"><div class="dot"></div><span>实时更新</span><button class="refresh-btn" id="soundBtn" onclick="toggleSound()" title="新订单提示音开关">🔔</button><button class="refresh-btn" onclick="refreshWithSound()">刷新</button><button class="refresh-btn" onclick="exportBackup()" style="margin-left:8px">导出备份</button></div></div></div>
<div class="ct">
    <div class="stats">
    ...
    <div class="sec"><div class="sh"><div class="stl">早餐订单 <span class="bdg" id="ob">0</span></div><select id="breakfastFilter" onchange="loadO()"><option value="1">今天</option><option value="3">近3天</option><option value="7" selected>近7天</option><option value="30">近30天</option></select></div><div class="cd" id="ol"><div class="em"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><p>暂无订单</p></div></div></div>
    <div class="sec"><div class="sh"><div class="stl">清洁请求 <span class="bdg" id="cb">0</span></div><select id="cleaningFilter" onchange="loadC()"><option value="1">今天</option><option value="3">近3天</option><option value="7" selected>近7天</option><option value="30">近30天</option></select></div><div class="cd" id="cl"><div class="em"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><p>暂无请求</p></div></div></div>
    <div class="sec"><div class="sh"><div class="stl">退房提醒 <span class="bdg" id="kb">0</span></div><select id="checkoutFilter" onchange="loadK()"><option value="1">今天</option><option value="3">近3天</option><option value="7" selected>近7天</option><option value="30">近30天</option></select></div><div class="cd" id="kl"><div class="em"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><p>暂无提醒</p></div></div></div>
</div>
</div>
<script>
let soundEnabled=true,lastO=0,lastC=0,lastK=0,token=localStorage.getItem('admin_token')||'';
function authHeaders(){return {'Content-Type':'application/json','Authorization':'Bearer '+token}}
function toggleSound(){soundEnabled=!soundEnabled;const b=document.getElementById('soundBtn');b.textContent=soundEnabled?'🔔':'🔕';b.style.opacity=soundEnabled?1:.6}
